...
function formatDate(date) {
  const newDate = new Date(date);
  const res = newDate.toLocaleString('en-US', { timeZone: 'UTC' });
  return res;
}
...
function getCountWeekendsInMonth(month, year) {
  const daysQnty = new Date(year, month, 0).getDate();
  let counter = 0;

  for (let i = 1; i <= daysQnty; i += 1) {
...
function getWeekNumberByDate(date) {
  ...
  const diffTime = Math.floor((date - startDate) / (24 * 60 * 60 * 1000));

  let firstDayWeekDay = startDate.getDay();
  if (firstDayWeekDay === 0) {
    firstDayWeekDay = 7;
  }
  const weekNumber = Math.ceil((diffTime + firstDayWeekDay) / 7);
